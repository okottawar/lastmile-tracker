...
async def create(payload:OrderCreate,current_user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db)):
    ...
    b=await calculate_charge(db,pickup_pincode=payload.pickup_pincode,drop_pincode=payload.drop_pincode,length_cm=payload.length_cm,breadth_cm=payload.breadth_cm,height_cm=payload.height_cm,actual_weight_kg=payload.actual_weight_kg,order_type=payload.order_type,payment_type=payload.payment_type)
